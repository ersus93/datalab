...
class EntradaForm(FlaskForm):
    ...
    # Cantidades
    cantidad_recib = DecimalField(_('Cantidad Recibida'), 
                                   validators=[DataRequired(), NumberRange(min=0.01)], 
                                   places=2)
    cantidad_muest = DecimalField(_('Cantidad Muestreada'), 
                                   validators=[Optional()], 
                                   places=2)
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.fabrica_id.choices = [(f.id, f"{f.nombre} ({f.cliente.nombre})") for f in
                                   Fabrica.query.filter_by(activo=True).join(Cliente).order_by(Fabrica.nombre).all()]
        self.producto_id.choices = [(p.id, p.nombre) for p in
                                    Producto.query.filter_by(activo=True).order_by(Producto.nombre).all()]
        self.rama_id.choices = [(0, _('-- Seleccionar --'))] + [(r.id, r.nombre) for r in
                                                                 Rama.query.order_by(Rama.nombre).all()]
