...
Este plan describe el proceso para pasar el código de la rama `dev` a `main`, excluyendo los directorios `docs/`, `plans/`, `ski/` y `utiles/`, publicar los cambios y verificar que no existan inconsistencias.
...
1. **Documentación interna**: Crear planes, docs, skills y datos en `docs/`, `plans/`, `ski/` y `utiles/` (rama `dev`)  
